...
namespace Simnet
{
  ...
  void SetN2kPGN6305(tN2kMsg &msg, const ApMsgType &mode, const PGN65305Payload &payload)
  {
    msg.SetPGN(65305UL);
  ...
  */
  ...
  void ParsePgn130850Ap(const tN2kMsg &N2kMsg, uint8_t &addr, Command &simnetcmd, ApStatus &apStatus, ApCommand &apcmd, ApDirection &dir, double &angle)
  {
    int index = 0;
    ...
    simnetcmd = static_cast<Command>(N2kMsg.GetByte(index)); // should be 255
    apStatus = static_cast<ApStatus>(N2kMsg.GetByte(index)); // unsure if it matters what this is
    apcmd = static_cast<ApCommand>(N2kMsg.GetByte(index));   // command to execute
    N2kMsg.GetByte(index);                                   // spare ?
    dir = static_cast<ApDirection>(N2kMsg.GetByte(index));
  ...
  void SetPGN65340(tN2kMsg &msg, const ApState &mode /*Mode63540 &mode*/)
  {
    // ac 12
    msg.SetPGN(65340UL);
    ...
    switch (mode)
    {
    case ApState::Standby:
      msg.AddByte(0x00);
      msg.AddByte(0xfe);
      msg.AddByte(0xf8);
      break;
    case ApState::Heading:
      msg.AddByte(0x01);
      msg.AddByte(0xfe);
      msg.AddByte(0xfa);
      break;
    // case ApState::Followup: // this cant be right
    case ApState::Wind:
      msg.AddByte(0x03);
      msg.AddByte(0xfe);
      msg.AddByte(0xfa);
  ...
  void SetPGN65341(tN2kMsg &msg, const ApState &state, const double &angle)
  {
    ...
    msg.Add2ByteDouble(angle, 0.0001);
  }
